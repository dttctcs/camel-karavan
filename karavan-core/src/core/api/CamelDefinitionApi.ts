...
export class CamelDefinitionApi {
    ...
    static createStreamCachingDefinition = (element: any) => {
        const def = element ? new StreamCachingDefinition({ ...element }) : new StreamCachingDefinition();
        def.uuid = element?.uuid ? element.uuid : def.uuid;
        console.log('stream caching:', def)
        if (!def.stepName) def.stepName = 'streamCaching';
        if (!def.id) def.id = '';
        if (!def.bufferSize) def.bufferSize = 0;
        if (!def.spoolEnabled) def.spoolEnabled = false;
        if (!def.spoolDirectory) def.spoolDirectory = '';
        if (!def.spoolThreshold) def.spoolThreshold = 0;
        return def;
    }
    ...
    static createStep = (name: string, body: any, clone: boolean = false): CamelElement => {
        ...
        switch (name) {
            ...
            case 'StreamCachingDefinition': return CamelDefinitionApi.createStreamCachingDefinition(newBody);
